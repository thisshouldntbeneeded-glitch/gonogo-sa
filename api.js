...
var GoNoGoAPI = (function() {
  ...
  async function checkBackend() {
    ...
    try {
      var res = await fetch('/api/health', { signal: AbortSignal.timeout(2000) });
      if (res.ok) { _backendAvailable = true; return true; }
    } catch(e) { /* no backend */ }
  ...
  return {
    ...
    async getStats() {
      ...
      if (typeof BRAND_DATA !== 'undefined') {
        var totalBrands = 0;
        BRAND_DATA.forEach(function(c) { totalBrands += c.brands.length; });
        return { totalCategories: BRAND_DATA.length, totalBrands: totalBrands, totalReviews: 0 };
      }
      return {};
    },
